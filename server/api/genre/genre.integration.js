...
describe('Genre API:', function() {
  ...
  describe('PUT /api/genres/:id', function() {
    ...
    it('should respond with the updated genre on a subsequent GET', function(done) {
      request(app)
        .get(`/api/genres/${newGenre._id}`)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          ...
          expect(genre.name).to.equal('Updated Genre');
          expect(genre.info).to.equal('This is the updated genre!!!');
  ...
  describe('PATCH /api/genres/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .patch(`/api/genres/${newGenre._id}`)
        .send([
          { op: 'replace', path: '/name', value: 'Patched Genre' },
          { op: 'replace', path: '/info', value: 'This is the patched genre!!!' }
        ])
        .expect(200)
        .expect('Content-Type', /json/)
        .end(function(err, res) {
    ...
    it('should respond with the patched genre', function() {
      expect(patchedGenre.name).to.equal('Patched Genre');
      expect(patchedGenre.info).to.equal('This is the patched genre!!!');
    });
  });
